...
local function CreateSettingsBlock(parent)
    ...
    -- Fade duration slider (0-15 seconds)
    do
        local rowHeight = 48
        local row = CreateFrame("Frame", nil, block)
        row:SetSize(block:GetWidth() - 20, rowHeight)
        row:SetPoint("TOPLEFT", block, "TOPLEFT", 10, y)

        local lbl = row:CreateFontString(nil, "OVERLAY", "GameFontNormal")
        lbl:SetPoint("TOPLEFT", row, "TOPLEFT", 6, -4)
        lbl:SetText(L["CFG_FADE_DURATION"] or "Fade Duration")
        lbl:SetTextColor(0.9, 0.9, 0.92)

        local desc = row:CreateFontString(nil, "OVERLAY", "GameFontNormalSmall")
        desc:SetPoint("TOPLEFT", lbl, "BOTTOMLEFT", 0, -2)
        desc:SetText("|cff555560" .. (L["CFG_FADE_DURATION_TT"] or "How long the fade animation takes (0 = instant).") .. "|r")
        desc:SetWidth(row:GetWidth() - 140)
        desc:SetJustifyH("LEFT")

        local valLabel = row:CreateFontString(nil, "OVERLAY", "GameFontNormal")
        valLabel:SetPoint("RIGHT", row, "RIGHT", -6, 8)
        valLabel:SetTextColor(ACCENT_R, ACCENT_G, ACCENT_B)

        local slider = CreateFrame("Slider", "HideAnythingFadeDurSlider", row, "OptionsSliderTemplate")
        slider:SetSize(120, 14)
        slider:SetPoint("RIGHT", row, "RIGHT", -6, -8)
        slider:SetMinMaxValues(0, 15)
        slider:SetValueStep(0.5)
        slider:SetObeyStepOnDrag(true)
        slider.Low:SetText("0s")
        slider.High:SetText("15s")
        slider.Low:SetTextColor(0.4, 0.4, 0.45)
        slider.High:SetTextColor(0.4, 0.4, 0.45)

        local curVal = HA:GetSetting("fadeDuration") or 0.3
        slider:SetValue(curVal)
        valLabel:SetText(string.format("%.1fs", curVal))

        slider:SetScript("OnValueChanged", function(self, value)
            value = math.floor(value * 2 + 0.5) / 2  -- snap to 0.5
            HA:SetSetting("fadeDuration", value)
            valLabel:SetText(string.format("%.1fs", value))
        end)
        ...
        sep:SetPoint("BOTTOMLEFT", row, "BOTTOMLEFT", 0, 0)
        sep:SetPoint("BOTTOMRIGHT", row, "BOTTOMRIGHT", 0, 0)
        sep:SetColorTexture(0.15, 0.15, 0.18, 1)

        y = y - rowHeight
    end

    y = MakeSettingsToggle(y, L["CFG_CHAT_HIDE_EDITBOX"] or "Hide Chat Typing Area",
        L["CFG_CHAT_HIDE_EDITBOX_TT"] or "Also hide the chat input box at the bottom when a chat frame is hidden.",
        function() return HA:GetSetting("chatHideEditBox") end,
        function()
            HA:SetSetting("chatHideEditBox", not HA:GetSetting("chatHideEditBox"))
            HA:ApplyChatEditBoxHide()
        end)

    y = MakeSettingsToggle(y, L["CFG_CHAT_SHOW_ON_ENTER"] or "Show Chat on Enter",
        L["CFG_CHAT_SHOW_ON_ENTER_TT"] or "Temporarily show hidden chat frames when you press Enter to type.",
        function() return HA:GetSetting("chatShowOnEnter") end,
        function()
            HA:SetSetting("chatShowOnEnter", not HA:GetSetting("chatShowOnEnter"))
        end)
